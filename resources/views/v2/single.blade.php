                        ...
                        <span class="badge"><div class="product-rating">
                            <i class="lni lni-star-filled" style="color:#FDB526"></i> </div></span>
                                        <a class="product-thumbnail d-block" href="#">
                                            <img
                                                src="{{$model->Images->first()?asset($model->Images->first()->path):""}}"
                                                alt=""></a>
                                    </div>
                                    <div class="product-description">
                                        <p class="mb-0">{{Str::limit($model->bio, 60 )}}</p>
                                        <form
                                            action="{{route('ProviderServices',['username'=>$provider->username,'model'=>$model->id])}}">
                                            <ul class="card-list-checkbox">
                                                @foreach($model->Services as $service)
                                                    <li><input type="radio" name="service" value="{{$service->id}}"
                                                               required id="">{{$service->title}} -
                                            ...
                                            <button class="btn btn-success btn-sm  form-control sp-btn" href="#">{{__('site.Book Now')}}
                                            </button>
                                        </form>
                                    </div>
                                </div>
