...
@section('content')
    ...
    <div class="toast pwa-install-alert shadow" id="pwaInstallToast" role="alert" aria-live="assertive"
        ...
        <div class="toast-body">
            <button class="ml-3 close" type="button" data-dismiss="toast" aria-label="Close"><span aria-hidden="true">&times;</span>
            </button>
            ...
            <span class="mb-0 d-block text-white">Add Suha on your mobile home screen. Click the<strong class="mx-1">"Add to Home Screen"</strong>button & enjoy it like a regular app.</span>
        </div>
    </div>
                    ...
                    <div class="cta-area ">
                        <div class="container">
                            <div class="cta-text p-4 p-lg-5"
                                 style="background-image: url({{asset($provider->Services->where('is_hot_deal',true)->first()->Images->first()->path)}})">
                    ...
                    <span class="badge"><div class="product-rating">
                      <i class="lni lni-star-filled" style="color:#FDB526"></i> 4.88 (39)</div></span>
                                            <a class="product-thumbnail d-block"
                                               href="{{route('ProviderCategory',[$provider->username,$category->id])}}">
                                                <img
                                                    src="{{$category->Images->first()?asset($category->Images->first()->path):""}}"
                                                    alt=""></a>
                                        </div>
                                        <div class="product-description">
                                            <a class="product-title d-block"
                                               href="{{route('ProviderCategory',[$provider->username,$category->id])}}">{{$category->title}}</a>
